...
test("m:n collections on owning side and no items [mongo]", async () => {
  const authors = await orm.em.findAll(Author);
  expect(authors[1].books.isInitialized()).toBe(true);
  expect(authors[1].books.isInitialized(true)).toBe(true);
  expect(authors[1].books).toHaveLength(0);
});
